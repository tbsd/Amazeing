...
namespace MazeBuilderNs {
  ...
  public class MazeBuilder {
    ...
    public void BinaryTree() {
      ...
      // reomove all walls on borers of uv
      for (int posY = 0; posY < ActualHeight; ++posY) {
        for (int posX = 0; posX < ActualWidth; ++posX) {
          MazeElement cur = maze[posY, posX];
          if (cur != null && cur.State == NodeState.Wall) {
            if ((posY == 0 || maze[posY - 1, posX] == null) && maze[posY + 1, posX] is Node)
              cur.State = NodeState.Path;
            if ((posX == 0 || maze[posY, posX - 1] == null) && maze[posY, posX + 1] is Node)
              cur.State = NodeState.Path;
            if ((posY == ActualHeight - 1 || maze[posY + 1, posX] == null) && maze[posY - 1, posX] is Node)
              cur.State = NodeState.Path;
            if ((posX == ActualWidth - 1 || maze[posY, posX + 1] == null) && maze[posY, posX - 1] is Node)
              cur.State = NodeState.Path;
          }
        }
      }
      // actual binary tree algorithm
      for (int posY = 0; posY < ActualHeight - 1; ++posY) {
        for (int posX = 0; posX < ActualWidth - 1; ++posX) {
          MazeElement cur = maze[posY, posX];
          if (cur is Node) {
            MazeElement bottom = maze[posY + 1, posX];
            MazeElement right = maze[posY, posX + 1];
            if (bottom != null && right != null) {
              if (bottom.State == NodeState.Wall && right.State == NodeState.Wall) {
                if (rng.Next(0, 2) == 0)
                  bottom.State = NodeState.Path;
                else 
                  right.State = NodeState.Path;
              } else {
                if (bottom.State == NodeState.Border)
                  right.State = NodeState.Path;
                else if (right.State == NodeState.Border)
                  bottom.State = NodeState.Path;
              }
            } else {
              if (bottom == null)
    ...
    public string toString() {
      ...
      for (int i = 0; i < actualHeight; i++) {
        for (int j = 0; j < actualWidth; j++) {
          if (maze[i, j] == null)
            str += "░";
          else if (maze[i, j].State == NodeState.Wall || maze[i,j].State == NodeState.Border)
            str += "▓";
          else
            str += "▒";
        }
